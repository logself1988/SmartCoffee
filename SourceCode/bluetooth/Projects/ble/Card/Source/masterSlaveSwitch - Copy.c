...
// Scan duration in ms
#define DEFAULT_SCAN_DURATION 550
...
#define DEFAULT_RSSI_PERIOD 200           //changed from 500ms to 200ms
...
static uint8 RSSI_Threshold = 60;
...
//  NULL                      // Pairing / Bonding state Callback (not used by
//  application)
//};
...
//  advertData[26] = newChar1[1];
  ...
  //    HalLcdWriteStringValue("major_0", advertData[25], 16, HAL_LCD_LINE_6);
  //    HalLcdWriteStringValue("major_1", advertData[26], 16, HAL_LCD_LINE_7);

  HalFlashRead(0x79, 0, newChar1, 1);
  RSSI_Threshold = newChar1[0];
  //    HalLcdWriteStringValue("Threshold",RSSI_Threshold, 16, HAL_LCD_LINE_6);

  HalFlashRead(0x7A, 0, newChar1, 1);
  ROLE_Flags = newChar1[0];
  //    HalLcdWriteStringValue("Role", ROLE_Flags , 16, HAL_LCD_LINE_7);
...
uint16 MasterSlaveSwitch_ProcessEvent(uint8 task_id, uint16 events)
{
  ...
  if (events & MSS_START_DEVICE_EVT) {
    ...
    if (ROLE_Flags == 1) {
      osal_start_timerEx(masterSlaveSwitch_TaskID, MSS_SWITCH_MODE, 15000);
    }
    // Start the Device
...
//          HalLcdWriteString( "Card Initialized", HAL_LCD_LINE_1 );
...
////    {
//      if (pEvent->deviceInfo.eventType == GAP_ADRPT_SCAN_RSP) // adv
//    {
//
//      if (pEvent->deviceInfo.pEvtData[9] == 0x4A &&
//          pEvent->deviceInfo.pEvtData[10] == 0xA8 &&
//          pEvent->deviceInfo.pEvtData[11] == 0x31) {
//
////            if((65536 - pEvent->deviceInfo.rssi) < RSSI_Threshold){
//
...
//            if((65536 - pEvent->deviceInfo.rssi) < RSSI_Threshold){
//      if(pEvent->deviceInfo.pEvtData[1] == 51 && pEvent->deviceInfo.pEvtData[2] == 54 && pEvent->deviceInfo.pEvtData[3] == 56){ // 3 6 8
        simpleBLEAddDeviceInfo(pEvent->deviceInfo.addr,
                               pEvent->deviceInfo.addrType);
        ...
        for (i = 0; i < pEvent->deviceInfo.dataLen; i++) {
          if (pEvent->deviceInfo.pEvtData[i] == 'a' &&
              pEvent->deviceInfo.pEvtData[i + 1] == 'a' &&
                pEvent->deviceInfo.pEvtData[i + 2] == 'a' &&
                  pEvent->deviceInfo.pEvtData[i + 3] == 'a')
             break;
        }
        if (i < pEvent->deviceInfo.dataLen) {
        i += 4;
        tableMinor[simpleBLEScanRes - 1] =
            (pEvent->deviceInfo.pEvtData[i] - '0') * 1000 + 
            (pEvent->deviceInfo.pEvtData[i + 1] - '0') * 100 + 
            (pEvent->deviceInfo.pEvtData[i + 2] - '0') * 10 + 
            (pEvent->deviceInfo.pEvtData[i + 3] - '0') * 1;
        } else {
        tableMinor[simpleBLEScanRes - 1] = pEvent->deviceInfo.dataLen;
        }
//      }
