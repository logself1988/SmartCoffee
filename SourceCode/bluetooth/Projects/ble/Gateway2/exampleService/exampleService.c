...
static uint8 simpleProfileChar1UserDesp[] = "HOST\0";
...
static uint8 simpleProfileChar2UserDesp[] = "PORT\0";
...
static uint8 simpleProfileChar3UserDesp[] = "SSID\0";
...
static uint8 simpleProfileChar4UserDesp[] = "PASSWORD\0";
...
static void simpleProfile_HandleConnStatusCB(uint16 connHandle,
                                             uint8 changeType);
...
//      osal_memset(simpleProfileChar3, 0, SIMPLEPROFILE_CHAR3_LEN);
...
//      osal_memset(simpleProfileChar4, 0, SIMPLEPROFILE_CHAR4_LEN);
...
//  return (ret);
  ...
  switch ( param )
  {
    case SIMPLEPROFILE_CHAR1: osal_memcpy( simpleProfileChar1, value, 4 ); break;
    case SIMPLEPROFILE_CHAR2: osal_memcpy( simpleProfileChar2, value, 2  ); break;
    case SIMPLEPROFILE_CHAR3: osal_memcpy( simpleProfileChar3, value, 20  ); break;
    case SIMPLEPROFILE_CHAR4: osal_memcpy( simpleProfileChar4, value, 20  ); break;
...
//  return (status);
  ...
  if ( pAttr->type.len == ATT_BT_UUID_SIZE )
  {
    ...
    switch ( uuid )
    {
      case SIMPLEPROFILE_CHAR1_UUID: *pLen = 4; osal_memcpy( pValue, pAttr->pValue, 4 ); break;
      case SIMPLEPROFILE_CHAR2_UUID: *pLen = 2; osal_memcpy( pValue, pAttr->pValue, 2 ); break;
      case SIMPLEPROFILE_CHAR3_UUID: *pLen = 20; osal_memcpy( pValue, pAttr->pValue, 20 ); break;
      case SIMPLEPROFILE_CHAR4_UUID: *pLen = 20; osal_memcpy( pValue, pAttr->pValue, 20 ); break;
        
      default: *pLen = 0; status = ATT_ERR_ATTR_NOT_FOUND; break;
    }
  }
  else
  {
...
//      }
      ...
      // Write the value
      if (len <= SIMPLEPROFILE_CHAR3_LEN) {
        osal_memset(pAttr->pValue,0,SIMPLEPROFILE_CHAR3_LEN);
...
//      }
      ...
      // Write the value
      if (len <= SIMPLEPROFILE_CHAR4_LEN) {
        osal_memset(pAttr->pValue,0,SIMPLEPROFILE_CHAR4_LEN);
        osal_memcpy( pAttr->pValue, pValue, len );
